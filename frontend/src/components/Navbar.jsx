  ...
  return (
    <nav className="absolute top-0 left-0 w-full py-6 z-10 border-t-[5px] border-[#00A3FF]">
      <div className="max-w-[1200px] mx-auto px-6 flex justify-between items-center">
        <Link to="/" className="text-2xl font-bold text-text-light tracking-tighter">swadSetu</Link>
        ...
        <div className="flex items-center gap-5">
          ...
          <Link to="/dashboard" className="text-text-light text-xl flex items-center justify-center"><User size={20} /></Link>
          <Link to="/dashboard" className="bg-primary hover:bg-primary-hover text-white px-6 py-2.5 rounded font-semibold text-sm transition-colors text-center">Order Now</Link>
        </div>
      </div>
    </nav>
  );
